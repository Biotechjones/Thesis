...
This document provides a quick\upcite{jones2010vivo} set of instructions for using the
\class{ubcdiss} class to write a dissertation in \LaTeX. 
Unfortunately this document cannot provide an introduction to using
\LaTeX.  The classic reference for learning \LaTeX\ is
\citeauthor{lamport-1994-ladps}'s
book\upcite[.]{lamport-1994-ladps} There are also many freely-available
tutorials online;
...
\cref{sec:SuggestedThesisOrganization}.
We then cover the ease of managing cross-references in \LaTeX\ in
\cref{sec:CrossReferences}.
We cover managing and using bibliographies with \BibTeX\ in
\cref{sec:BibTeX}. 
We briefly describe typesetting attractive tables in
\cref{sec:TypesettingTables}.
We briefly describe including external figures in
\cref{sec:Graphics}, and using special characters and symbols
in \cref{sec:SpecialSymbols}.
As it is often useful to track different versions of your dissertation,
we discuss revision control further in
\cref{sec:DissertationRevisionControl}. 
...
The \ac{UBC} \acf{GPS} specifies a partfoocular arrangement of the
components forming a thesis.
...
will be typeset ( bold, or italics) from the meaning of that
something. 
...
and \emph{id est} (\verb+\eg+ and \verb+\ie+), 
...
package's\ \verb+\cref{}+ command
makes it easier still. 

A thing to be cross-referenced, such as a section, figure, or equation,
is \emph{labelled} using a unique, user-provided identifier, defined
using the \verb+\label{}+ command.  
The thing is referenced elsewhere using the \verb+\cref{}+ command.
For example, this section was defined using:

\begin{quote}
    We then cover the ease of managing cross-references in \LaTeX\
    in \cref{sec:CrossReferences}.
\end{quote}
...
documentation for \texttt{crefname}.
...
Each reference must be described by a unique \emph{key}.
...
\begin{quote}
   present examples of crosscutting 
...
% There are times that you don't want to use \cref{}
...
the figure is used, the text should use \verb+\cref+ to reference
the figure's label.
\cref{fig:latex-affirmation} is an example of a figure.
\begin{figure}
    \centering
...
\item The \verb+\label+ command provides for associating a unique, user-defined,
    ...
    as described in \cref{sec:CrossReferences}.
\end{enumerate}
