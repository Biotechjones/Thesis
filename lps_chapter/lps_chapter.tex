...
\label{sec:results}
	...
	\label{sub:o_antigen_structure_determination_ps1_}
		...
		\begin{table}[h]  % LPS OPS
			\centering
			\caption{\Iac{NMR} data for \caulobacter \ac{PS}1 (40\cel) and deacylated \ac{PS}1 (50\cel). Me at 3.62/61.3 ppm.}
			\label{tbl:lpsops}
...
\label{sec:discussion}
	...
	An additional aspect to this \ac{LPS} it the fact that its O-antigen is of homogenous length. While other \ac{LPS}s vary in size due to the number of O-antigen repeat groups, appearing as a laddering of bands by \ac{SDS-PAGE}, the \ac{LPS} from C. crescentus appears as a single band\upcite[.]{walker94} Initial \ac{MALDI-TOF} analysis of the entire \ac{LPS} indicates a size of about 10.8 kDa (see \cref{fig:lpsmalditof} on \cpageref{fig:lpsmalditof}).
