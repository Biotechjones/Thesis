...
All samples were boiled for 5 minutes in sample buffer prior to applying them to gel. Sample buffer is made as a 4X concentrate comprising 200 mM Tris pH 6.8, 8\% sodium dodecyl sulfate, 40\% glycerol, 4\% $\beta$-mercaptoethanol, 0.25\% bromophenol blue.
    ...
    Western blotting of \ac{LPS} followed standard laboratory blotting techniques\upcite[.]{towbin1979electrophoretic} \ac{SDS-PAGE} gels of \ac{LPS} were electro-blotted on to a nitrocellulose membrane which subsequently agitated in Tris-buffered milk blocking buffer for one hour. Occasionally, spot-blot westerns were performed. Spot-blot westerns forgo the \ac{SDS-PAGE}; the sample is spotted directly on to the membrane and allowed to dry.
...
\subsection{HEK-Blue h\ac{tlr4} assay}\label{sec:hek-blue-htlr4}
...
We attempted to isolate the core oligosaccharide-\ac{OPS} fraction from the \ac{LPS} prior to starting \ac{NMR} analysis. Hydrolysis of the ketosidic bond between Kdo and lipid A (see the arrow in \cref{fig:lipidA}) was carried out with one of the classical protocols, 0.1 \si{\molar} \ce{HCl} at 100\cel for one hour. The reaction caused the regular precipitated lipid A, but the supernate had no detectable \ac{OPS}. We used spot-blot westerns and far-westerns to detect the presence of \ac{OPS}, because it was the structure bound by RsaA that was of most interest to us. Milder hydrolysis conditions, acetic acid or \textsc{sds} accelerated\upcite[,]{caroff1988detergent} also produced no detectable \ac{OPS}. In the concern that the \ac{OPS} didn't immobilize to the membrane, we tried to chemically link the polysaccharide to amine-coated surface using carbodiimide chemistry\upcite[]{jauho2000new}; the chemical linkages failed to demonstrate the presence of free-\ac{OPS}. We had no evidence to conclude that our \ac{OPS} was labile acid degradation. At the time the only clue about the structure of the \ac{OPS} was the presence of N-acetylperosamine and persoamine containing \ac{OPS} have no inherent instability\upcite[.]{caroff1984antigenic, perry1986structure} It was this curious situation that was the main impetuous to pursue a full structural solution.

Attempts  to accurately measure the molecular weight of our intact smooth-\ac{LPS} were partly successful. Getting accurate \ac{MALDI-TOF} results for large molecular weight polysaccharides is at best difficult and the technology is most often used for the analysis of smaller oligosaccharides (as below) and lipid A molecules. \Cref{fig:lpsmalditof} shows our best results. Our mass spectrum shows a tight clustering of m/z peaks at or near 10.8 kDa (assuming a single negative charge). Like our electrophoresis data, our \ac{MALDI-TOF} data demonstrate that \caulobacter{} \ac{LPS} is not heterogeneous in length. The small differences in m/z between the peaks in \cref{fig:lpsmalditof} are around 180 Da, the size range of a single hexose molecule not an entire \ac{OPS} subunit.
...
polymer with well-defined repeating units. It was clear that an intact polymer was released from hydrolysis. The free-\ac{OPS} was detectable by \ac{NMR} but not spot-blot western (see above, \cref{sub:characterisation_of_whole_lps}) Attempts to separate this material by anion-exchange
...
None of the determined carbohydrate structures give any clues to why we were unable to detect free-\ac{OPS} by spot-blot western (see \cref{sub:characterisation_of_whole_lps}). On the contrary, the \ac{NMR} signals unambiguously show that the core oligosaccharide and \ac{OPS} stay intact after mild-acid hydrolysis. We do not have much information about which structures in the \caulobacter \ac{LPS} are bound by RsaA or the antibodies used in western blotting. Genetic knockouts point towards the requirement of N-acetylpersoamine, but now that the complete structure has been determined it is not clear why the N-acetylpersoamine containing structures (\ac{OPS} and end caps) were not previously detectable.
